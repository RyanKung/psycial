...
impl BertClassifier {
    ...
    pub fn train(&mut self, texts: &[String], labels: &[String]) -> Result<(), Box<dyn Error>> {
        ...
        let selected_psy: Vec<Vec<f64>> = all_psy.iter().map(|f| selector.transform(f)).collect();
        ...
        self.fusion_layer
            .learn_weights(&selected_psy, &bert_features, labels);
    ...
    fn train_on_fused(&mut self, fused: &[Vec<f64>], labels: &[String]) {
        ...
        let normalized: Vec<Vec<f64>> = fused.iter().map(|f| self.l2_normalize(f)).collect();
        ...
        for class in counts.keys() {
            ...
            self.class_centroids
                .insert(class.clone(), normalized_centroid);
        }
    }
