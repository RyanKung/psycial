...
impl FullPsyAttentionClassifier {
    ...
    pub fn train(&mut self, texts: &[String], labels: &[String]) {
        ...
        println!(
            "Extracting 930 psychological features from {} samples...",
            texts.len()
        );
        ...
        println!(
            "Feature selection complete: {} features selected",
        ...
        let selected_features: Vec<Vec<f64>> = all_features
            .iter()
            .map(|features| self.feature_selector.as_ref().unwrap().transform(features))
            .collect();
        ...
        let encoded_features: Vec<Vec<f64>> = selected_features
            .iter()
            .map(|features| self.attention_encoder.as_ref().unwrap().encode(features))
            .collect();
...
mod tests {
    ...
    fn test_full_pipeline() {
        ...
        let labels = vec!["ENFP".to_string(), "INFJ".to_string(), "ENTJ".to_string()];
