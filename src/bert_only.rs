...
pub fn main_bert_only(_args: Vec<String>) -> Result<(), Box<dyn Error>> {
    ...
    // Normalize features
    let normalized_train: Vec<Vec<f64>> = train_features.iter().map(|f| l2_normalize(f)).collect();
    ...
    let normalized_test: Vec<Vec<f64>> = test_features.iter().map(|f| l2_normalize(f)).collect();
